...
coin = 'ADA'
selected_indices = [97, 19, 22, 23, 67, 72, 94, 95, 98, 100, 111, 112]
timestep = 35
hidden_dim = 32
num_layers = 2
dropout_rate = 0.1
...
class lstm_short_tame(Strategy):
    def __init__(self):
        super().__init__()
        self.model = BiLSTMClassifierWithAttention(input_dim, hidden_dim, num_layers, 3, dropout_rate)
        model_filename = '/Users/duncanmaclennan/Documents/Jesse/trade/my-bot/strategies/lstm_short_tame/ADA_TOP_trained_model_lstm_index_[97, 98, 67, 100, 72, 111, 112, 19, 22, 23, 94, 95].pth'
        if model_filename:
        ...
        scaler_params_filename = '/Users/duncanmaclennan/Documents/Jesse/trade/my-bot/strategies/lstm_short_tame/ADA_scaler_params_index_[97, 98, 67, 100, 72, 111, 112, 19, 22, 23, 94, 95].joblib'
    ...
    def go_long(self):
        qty = utils.size_to_qty(200, self.price, fee_rate=self.fee_rate)
    ...
    def update_position(self):
        ...
        with torch.no_grad():
            prediction = self.model(features)
            _, predicted_class = torch.max(prediction, 1)
            print(f"predicted_class: {predicted_class.item()}")
        if self.is_short and predicted_class.item() == 2:
            self.liquidate()
